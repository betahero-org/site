<script lang="ts">
  import { externalLink, PageSection } from "$lib";
  import { Button, IconButton } from "fluent-svelte";
  import { links } from "$data/links";
  import Discord from "$static/ui/icons/discord.svg?raw";
  import Instagram from "$static/ui/icons/instagram.svg?raw";
  import Github from "$static/ui/icons/github.svg?raw";
  import Facebook from "$static/ui/icons/facebook.svg?raw";
  import Twitter from "$static/ui/icons/twitter.svg?raw";
  import Youtube from "$static/ui/icons/youtube.svg?raw";
  import Tiktok from "$static/ui/icons/tiktok.svg?raw";

  const contributors = [
    {
      name: "Tropical",
      link: "https://github.com/Tropix126/"
    },
    {
      name: "TheOnlyTails",
      link: "https://www.theonlytails.com/"
    },
    {
      name: "Yair",
      link: "https://github.com/yaichenbaum/"
    },
    {
      name: "Marcel Wagner",
      link: "https://github.com/chingucoding/"
    },
    {
      name: "Michael West",
      link: "https://michaelwe.st/"
    }
  ];
</script>

<PageSection type="footer" id="page-footer">
  <div class="column">
    <a class="logo" href="/" sveltekit:prefetch>
      <picture>
        <source media="(prefers-color-scheme: dark)" srcset="/branding/logo-dark.svg">
        <source media="(prefers-color-scheme: light)" srcset="/branding/logo-light.svg">
        <img alt="BetaHero logo" height="32" src="/branding/logo-light.svg" width="32">
      </picture>
      BetaHero
    </a>
    <div class="social-links">
      <IconButton
        href="https://facebook.com/{links.facebook}/"
        title="Facebook profile"
        aria-label="Facebook profile"
        {...externalLink}
      >
        {@html Facebook}
      </IconButton>
      <IconButton
        href="https://instagram.com/{links.instagram}/"
        title="Instagram profile"
        aria-label="Instagram profile"
        {...externalLink}
      >
        {@html Instagram}
      </IconButton>
      <IconButton
        href="https://youtube.com/channel/{links.youtube}/"
        title="Youtube profile"
        aria-label="Youtube profile"
        {...externalLink}
      >
        {@html Youtube}
      </IconButton>
      <IconButton
        href="https://tiktok.com/{links.tiktok}/"
        title="TikTok profile"
        aria-label="TikTok profile"
        {...externalLink}
      >
        {@html Tiktok}
      </IconButton>
      <IconButton
        href="https://twitter.com/{links.twitter}/"
        title="Twitter profile"
        aria-label="Twitter profile"
        {...externalLink}
      >
        {@html Twitter}
      </IconButton>
      <IconButton
        href="https://dsc.gg/{links.discord}"
        title="Discord server"
        aria-label="Discord server"
        {...externalLink}
      >
        {@html Discord}
      </IconButton>
      <IconButton
        href="https://github.com/{links.github.repo}/{links.github.siteRepo}/"
        title="GitHub repository"
        aria-label="GitHub repository"
        {...externalLink}
      >
        {@html Github}
      </IconButton>
    </div>
    <p>Copyright &copy; 2021 - {new Date().getFullYear()}, BetaHero</p>
    <a href="https://vercel.com/?utm_source=FilesCommunity&utm_campaign=oss" {...externalLink}>
      <img src="/branding/vercel-banner.svg" alt="Powered by Vercel" />
    </a>
  </div>
  <div class="column">
    <p>Pages</p>
    <Button variant="hyperlink" sveltekit:prefetch href="/">
      Home
    </Button>
    <Button variant="hyperlink" sveltekit:prefetch href="/docs">
      Documentation
    </Button>
    <Button variant="hyperlink" sveltekit:prefetch href="/blog">
      Blog
    </Button>
  </div>
  <div class="column">
    <p>Contribute to BetaHero</p>
    <Button variant="hyperlink" {...externalLink}
            href="https://github.com/{links.github.repo}/{links.github.siteRepo}/issues/new"
    >
      Give Feedback
    </Button>

    <Button variant="hyperlink" href="/docs/contributing/code-style">
      Style Guide
    </Button>

    <Button variant="hyperlink" href="/docs/contributing/translating-files">
      Translations
    </Button>

    <p>Contribute to this site</p>
    <Button variant="hyperlink" {...externalLink}
            href="https://github.com/{links.github.repo}/{links.github.siteRepo}/"
    >
      GitHub Repo
    </Button>

    <Button variant="hyperlink" {...externalLink}
            href="https://github.com/{links.github.repo}/{links.github.siteRepo}/issues/new"
    >
      Found a bug?
    </Button>
  </div>
  <div class="column">
    <p>Web Team</p>
    {#each contributors as contributor}
      <Button variant="hyperlink" href={contributor.link} {...externalLink}>{contributor.name}</Button>
    {/each}
  </div>
</PageSection>

<style lang="scss">
  @use "./Footer";
</style>
